...
$(function() {
	...
	$('.clientlist, .vacancies, .avatar_list').cleanWS();
	$('.clientlist .valignw img').clientDescr();
	 $('#er').randombg(); // смена бэка на 404 
}); // DOM loaded
	...
	$.fn.innermenu = function(options) {
		...
		return this.each(function() {
			...
			function getWidthForInner() {
				var w = 0,
					w2 = parseInt(ul2.width()) <= 118 ? 0 : parseInt(ul2.width()); // opera bugfix
				li1.each(function() {
					w += parseInt($(this).outerWidth())
				});
				w = parseInt(ul1.width()) + w2 - w - 20;
				return w;
			};
